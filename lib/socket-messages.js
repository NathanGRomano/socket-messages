...
function SocketMessages () {
  ...
  if (!(this instanceof SocketMessages)) return new SocketMessages();
  ...
  this.onMessage = function (socket, args) {
    ...
    self.actor(socket, function (err, actor) {
      ...
      self.target(socket, args, function (err, target) {
        ...
        event.target = target;
        event.content = args;
        self.exchange().emit('message', event, socket);
      });
    });
  };
...
SocketMessages.prototype.dettach = function (io) {
  if (io.sockets.fns.indexOf(this.middleware) >= 0) {
    io.sockets.fns.splice(io.sockets.fns.indexOf(this.middleware), 1);
  }
...
SocketMessages.prototype.autoPropagate = function (v) {
  if (typeof v === 'boolean') {
